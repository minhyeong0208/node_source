...
async function fetchHousingData() {
    const response = await fetch('https://raw.githubusercontent.com/selva86/datasets/master/BostonHousing.csv');  // 이 주소에서 데이터를 다운로드 받아 학습 시작
    ...
    const parsedData = rows.map(row => {
        ...
        return {
            ...
            medv: parseFloat(cols[13])   // 중간 주택 가격 (MEDV) -> 종속 변수
        };
    });
    return parsedData;
}
...
async function run() {
    ...
    await model.fit(tensorX, tensorY, { // 학습
        epochs: 500,
        batchSize: 32, // 배치 사이즈 추가 -> 데이터가 많으면 4의 배수로 늘려서 학습시킨다.
